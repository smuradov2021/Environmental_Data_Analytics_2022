...
```{r}
HighTemp<-c(44,48,43,50,49,45,46,48,51,45)
LowTemp<-c(30,31,33,35,34,39,29,32,36,37)
```
...
HighCel<-FToCel(HighTemp)
LowCel<-FToCel(LowTemp)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
Forecast<-data.frame("High Temperature in Farenheit"=HighTemp, "Low Temperature in Farenheit"=LowTemp,"High Temperature in Celcius"=HighCel,"Low Temperature in Celcius"=LowCel)
...
```{r}
summary(Forecast)
sd(Forecast$High.Temperature.in.Farenheit)
sd(Forecast$Low.Temperature.in.Farenheit)
sd(Forecast$High.Temperature.in.Celcius)
sd(Forecast$Low.Temperature.in.Celcius)
```
